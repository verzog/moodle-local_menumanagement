...
/*
    var icons = $.map($.map(document.styleSheets,
                function(s) {
            ...
            function(r) {
                var result = [];
                if (typeof r.cssText == 'string' && r.cssText.indexOf('::before { content: ') != -1) {
                    $.each(r.cssText.split(','),
                        function (i, item) {
                            var tmpItem = item.split(':')[0].trim().substring(1);
                            tmpItem = "fa "+tmpItem;
                            result.push(tmpItem);
                        });
                }
...
//        theme: 'fip-darkgrey'
    ...
    var nestObj = $('#nestable').nestable({
        group: 1,
        maxDepth : 2
    });
    
    //nestObj.on('change', updateOutput);
    ...
    var updateOutput = function(e)
    {
        ...
        if (window.JSON) {            
            ...
            var itemArr = {};            
            var cnt = 0;            
            var parents = $('#menu-id').children();
            parents.each(function(i) {
                if(!$(this).hasClass('new-item-only'))
                {
                    itemArr[cnt] = {};
                    itemArr[cnt]['id'] = $(this).attr('data-id');
                    itemArr[cnt]['label'] = $(this).attr('data-label');
                    ...
                    if($(this).find('ol'))
                    {
                        ...
                        var childs = $(this).find('ol').children();
                        if(childs.length > 0)
                        {
                            itemArr[cnt]["child"] = {};
                            childs.each(function(i) {                                       
                                itemArr[cnt]["child"][childCnt] = {}; 

                                itemArr[cnt]["child"][childCnt]['id'] = $(this).attr('data-id');
                                ...
                                if($(this).attr('data-icon') != 'fip-icon-block')
                                    ...
                                    itemArr[cnt]["child"][childCnt]['icon'] = '';
            ...
            output.val(window.JSON.stringify(itemArr));
...
//        var linkChanged = 0;
//        
//        if($(this).hasClass('label-text'))
//            labelChanged = 1;
//        if($(this).hasClass('link-text'))
//            linkChanged = 1;
//        
...
//            
//            if(labelChanged) {
//                var $p = $('<span class="label-text" data-id='+itemID+' data-editable />').text( $input.val() );
//                //$("li[data-id='" + itemID +"']").data('label',$input.val()) ;
//                $("li[data-id='" + itemID +"']").attr('data-label',$input.val());
//            }
//            
//            if(linkChanged) {
//                var $p = $('<span class="link-text" data-id='+itemID+' data-editable />').text( $input.val() );    
//                //$("li[data-id='" + itemID +"']").data('link',$input.val()) ;
//                $("li[data-id='" + itemID +"']").attr('data-link',$input.val());
//            }
//            
//            $input.replaceWith( $p );
//          
//          //$("li[data-id='" + itemID +"']").data('label',) ;
//          
//        };
//
//        /**
...
//    });
    ...
    $('#nestable-menu').on('click', function(e)
    {
        ...
        if (action === 'expand-all') {
            nestObj.nestable('expandAll');
        }
        if (action === 'collapse-all') {
            nestObj.nestable('collapseAll');
        }
    });
