 ...
 * @return array
 */
function local_menumanagement_get_all_childrenkeys(navigation_node $navigationnode) {
    ...
    if (count($navigationnode->children) == 0) {
        ...
        foreach ($childrennodeskeys as $ck) {
            $allchildren = array_merge($allchildren, local_menumanagement_get_all_childrenkeys($navigationnode->get($ck)));
        }
 ...
 * This uses json as its input and create menu nodes from its extraction
...
function local_menumanagement_build_custom_nodes_all($customnodes, navigation_node $node,
        ...
        foreach($menuItems as $menuItem)
        {
            ...
            if($nodevisible)
            {
                ...
                if (!$nodeischild) {
                    ...
                    $userprefcustomnode = get_user_preferences('local_menumanagement-collapse_'.$nodekey.'node', $collapsedefault);
                ...
                if(isset($menuItem->child))
                {
                    ...
                    {   
                        ...
                        $userprefcustomnode = get_user_preferences('local_menumanagement-collapse_'.$lastparentnode->key.'node',
                                $collapsedefault);
                        if ($userprefcustomnode == 1) {
 ...
 * @return array
 */
function local_menumanagement_build_custom_nodes($customnodes, navigation_node $node,
        $keyprefix='localmenumanagementcustom', $showinflatnavigation=true, $collapse=false,
        $collapsedefault=false) {
    global $USER;
    ...
    foreach ($lines as $line) {
                ...
                if (!empty($setting)) {
                    switch ($i) {
                        ...
                        case 3:
                            ...
                            $nodevisible &= local_menumanagement_cohort_is_member($USER->id, $setting);
        ...
        if ($nodevisible) {
            ...
            {
                if($nodeischild)
                    $nodetitle = "<span class='menu-title'>&nbsp;&nbsp;&nbsp;&nbsp;$nodetitle</span>";
                else
                    $nodetitle = "<span class='menu-title'>&nbsp;&nbsp;&nbsp;$nodetitle</span>";
            }
 ...
 * @return bool
 */
function local_menumanagement_cohort_is_member($userid, $cohortidnumber) {
    global $DB;
